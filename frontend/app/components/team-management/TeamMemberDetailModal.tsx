...
export default function TeamMemberDetailModal({ member, isOpen, onClose }: TeamMemberDetailModalProps) {
  ...
  // Reset image error when member changes
  useEffect(() => {
    setImageError(false);
  }, [member?._id]);
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="flex items-start gap-2 p-3 bg-warning/10 border border-warning/20 rounded-lg">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400">
              Privacy Notice: Sensitive personal information (salary, personal contact details, etc.) is hidden per company policy (BR 18b).
            </p>
          </div>
        </div>
      </div>
