...
export default function FreezePayrollPage() {
  ...
  const handleFreeze = async () => {
    ...
    setStatus(null);
    try {
      const res = await payrollExecutionService.freeze(runId);
      if (res.error) return setError(res.error);
      setStatus('Payroll frozen');
    } catch (e: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-xl font-semibold">Freeze Payroll</h1>
      ...
      <button className="bg-blue-600 text-white px-4 py-2 rounded" disabled={loading || !runId} onClick={handleFreeze}>
